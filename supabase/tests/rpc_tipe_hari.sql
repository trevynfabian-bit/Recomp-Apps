...
-- 7. Ganti fase aktif → target ikut berubah OTOMATIS, tanpa diketik ulang.
do $$
declare r record; k integer;
begin
  update public.profiles set fase_aktif = 'Cut'
   where user_id = '55555555-5555-5555-5555-555555555555';

  select target_kalori into k from public.v_tipe_hari_aktif where nama = 'Angkat Beban';
  assert k = 2350, format('setelah pindah Cut, target = %s, seharusnya 2350', k);

  select * into r from public.ambil_target_harian(date '2026-09-22');
  assert r.fase = 'Cut', format('fase = %s', r.fase);
  assert r.target_kalori = 2350, format('target = %s, seharusnya 2350', r.target_kalori);
end $$;
...
\echo 'Tipe hari & target OK — snapshot disegarkan, kolom lain utuh, fase mengubah target otomatis'
