...
console.log('\nPesan galat lapisan data (target, tipe hari, fase, berat, ukuran, tren, TDEE, budget, redistribusi, latihan, evaluasi, sumber data, impor, coach)');
{
  ...
  const kalimat = [];
  for (const berkas of ['src/data/target.ts', 'src/data/tipeHari.ts', 'src/data/fase.ts', 'src/data/beratPagi.ts', 'src/data/ukuran.ts', 'src/data/tren.ts', 'src/data/tdee.ts', 'src/data/budget.ts', 'src/data/redistribusi.ts', 'src/data/latihan.ts', 'src/data/evaluasi.ts', 'src/data/sumberData.ts', 'src/data/imporRiwayat.ts', 'src/data/coach.ts', 'src/data/akun.ts']) {
  ...
  cek('ada kalimat galat untuk diperiksa', kalimat.length >= 50, `hanya ${kalimat.length}`);
